...
void spi_config(SPI_Handler_t  ptrHandlerSPI){
	...
	/* 1. Activamos la señal de reloj del periférico */
	   ...
	   if(ptrHandlerSPI.SPI_Config.SPI_fullDupplexEnable == SPI_FULL_DUPPLEX){
		   ...
		   // Selecciona modo uni-direccional ( cada lina solo tiene una dirección )
	   ...
	   /* 8 Configuramos el formtato del dato (tamaño) para que sea de 8 bit */
	    ...
	    * de la otra forma, será el hadware el que controla la selleción del slave */
...
void spi_transmit(SPI_Handler_t ptrHandlerSPI, uint8_t * ptrData, uint32_t dataSize){
	...
	while(dataSize > 0){
		...
		// Actualizamos el puntero y el numero de datos que faltan para enviar
	 ...
	 * para esto debemos leer el DR y luego leer el SR del modulo SPI (paga 599)
	 */
...
void spi_receive(SPI_Handler_t ptrHandlerSPI, uint8_t * ptrData, uint32_t dataSize){
	...
	while(dataSize){
		...
		*ptrData = ptrHandlerSPI.ptrSPIx->DR;
		...
		// Actualizamos el puntero yy el tamaño de los datos
		ptrData++;
		dataSize--;
	}
}

/* Seleccionamos el esclavo llenado el pin SS a GND */
