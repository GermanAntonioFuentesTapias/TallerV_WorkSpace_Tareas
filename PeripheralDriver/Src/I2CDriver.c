...
void i2c_config(I2C_Handler_t *ptrHandlerI2C){
	...
	/* 3. Indicamos cual es la velocidad de reloj principal, que es la señal utilizada
	 * por el periférico para generar la señal de reloj para el bus I2C*/

	ptrHandlerI2C->ptrI2Cx->CR2   &= ~(0b111111 << I2C_CR2_FREQ_Pos); // Borramos las configuración previa
	ptrHandlerI2C-> ptrI2Cx->CR2 |= (MAIN_CLOCK_16_MHz_FOR_I2C << I2C_CR2_FREQ_Pos);
	  ...
	  * en esta configuración se incluye también la velocidad de reloj
	  * y el tiempo máximo para el cambio de la señal (T-Rise).
	  *  all comienza con los dos registros en 0	  */
	...
	if(ptrHandlerI2C->modeI2C == I2C_MODE_SM){
		...
		//Configuramos el registro que se encargga de generar la señal de reloj
...
void i2c_startTransaction(I2C_Handler_t *ptrHandlerI2C){
	...
	/* Verificamos que la línea no esta ocupada  - bit "busy" en I2C_CR2 */
...
void i2c_reStartTransaction(I2C_Handler_t *ptrHandlerI2C){
	...
	/* Mientas esperamos, el valor de SB es 0, entonces la negación (!) es 1 */
...
void i2c_sendSlaveAddressRW(I2C_Handler_t *ptrHandlerI2C, uint8_t slaveAddress , uint8_t readOrWrite){
	 ...
	 * debemos leer la secuencia, priemro el I2C_SR1 y luego I2C_SR2
	 */
...
void i2c_sendMemoryAddress(I2C_Handler_t *ptrHandlerI2C, uint8_t menAddr){

	/* 4. Enviamos la dirección de memoria que deseamos leer */
	ptrHandlerI2C->ptrI2Cx->DR = menAddr;
...
uint8_t i2c_readDataByte(I2C_Handler_t *ptrHandlerI2C){
	...
	/* 9 Esperamos hasta que el byyte entrante sea recibido */
