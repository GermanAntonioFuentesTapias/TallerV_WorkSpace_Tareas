 ...
 /******************************************************************************************************/
              ...
              if(posicionMenu == ACTIVAR_A){
            	  ...
            	  if(stateKeypad == TECLA_UNO){
							...
							LCD_sendSTR(&handlerLCD,"The password.");
							sprintf(dataLCD, "Nuevo valor = %u%u%u ", ContrasenaInicial[0],ContrasenaInicial[1],ContrasenaInicial[2] );
							LCD_setCursor(&handlerLCD,2,0);
							LCD_sendSTR(&handlerLCD,dataLCD);
							LCD_setCursor(&handlerLCD,3,1);
							LCD_sendSTR(&handlerLCD,"Atras con tecla D");
...
//				  posicionMenu = SALIO;
            	  ...
            	  // Aqui vendrian las condiciones para la alarma
            	 if(seleccionConfiguracion == CONTRASE){

            		 for(uint8_t j= 0; j < 3; j++){

            			Actualizar[j]= valorNumerosArreglo[0];
            			j++;
					...
					LCD_setCursor(&handlerLCD,1,0);
					LCD_sendSTR(&handlerLCD,"The password=");
					sprintf(dataLCD, "Nuevo valor = %s ",  Actualizar );
					LCD_setCursor(&handlerLCD,2,0);
					LCD_sendSTR(&handlerLCD,dataLCD);
					LCD_setCursor(&handlerLCD,3,1);
					LCD_sendSTR(&handlerLCD,"Atras con tecla D");
...
//			  Casos = SALIO;
			  ...
			  seleccionConfiguracion = CONTRASE;
			  posicionMenu = ACTIVAR_A; // Posicion donde me encuentro
			  Casos = SALIO;
			  estadoPantalla = MENUS;
			  stateKeypad = NOTHING;


			  // Ya tengo el arreglo almacenado, ahora que pasa menu = Activar
		  }
        	  }// Va guardar un (111)
