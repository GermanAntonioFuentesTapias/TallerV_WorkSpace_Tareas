	...
	while (1) {
		...
		if(rxData != '\0'){
			...
			if(rxData == 'c'){
				...
				startCounterTimer(&handlerAdcTimer);
			...
			// Paramos muestreo del ADC
				...
				StopCounterTimer(&handlerAdcTimer);
			}
			rxData = '\0';
		}
		...
		if(adcIsComplete == SET){
			...
			// sprint me convierte los numeros a string, la u significa un entero sin signo
			//freecodecamp ansi
			 sprintf(bufferData, "ADC: %u \n", adcValue);
			//dentro del bufferData creara que el valor de adcDato lo convierta en un string
